...
class TrainModel(object):
    ...
    def __init__(self, filename):
        ...
        try:
            ...
            return
        df2=pd.read_csv("failed_jobs/memory_csv/%s.csv"%filename)
        number_of_errors=df2.shape[0]
        if number_of_errors< self.df.shape[0]:
            self.df=self.df.sample(number_of_errors)
        # df3=pd.read_csv("failed_jobs/memory_csv/%s.csv"%filename)
        self.df["result"], df2["result"]="ok", "memory error"
        ...
        self.table="memory_classify_proportioned"
        ...
        if time_split:
            ...
            metrics={"toolid": filename, "num_instances":num_instances,
                    "accuracy": accuracy, "accuracy_time_split":accuracy2, "num_memory_errors":num_errors,
            ...
            command = '''INSERT INTO memory_classify_proportioned (toolid, num_instances, accuracy, accuracy_time_split, num_memory_errors, 
                        false_positives, false_negatives, false_positives_time_split, false_negatives_time_split)

                        VALUES (%(toolid)s, %(num_instances)s, %(accuracy)s, %(accuracy_time_split)s, %(num_memory_errors)s, %(false_positives)s, 
                        ...
                        num_memory_errors=%(num_memory_errors)s, false_positives=%(false_positives)s, false_negatives=%(false_negatives)s, 
            ...
            command = '''INSERT INTO memory_classify_proportioned (toolid, num_instances, accuracy, num_memory_errors) 
                            
                            VALUES (%(toolid)s, %(num_instances)s, %(accuracy)s, %(num_errors)s)
                            
                            ON DUPLICATE KEY UPDATE num_instances=%(num_instances)s, accuracy=%(accuracy)s, num_memory_errors=%(num_errors)s;
                            '''
        # print(tabulate(sorted([(k,v) for k,v in metrics.items()])))
...
toolids=getfiles("failed_jobs/memory_csv")
